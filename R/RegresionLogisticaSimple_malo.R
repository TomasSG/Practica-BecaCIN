...
ggplot(datos, aes(hora, cnt)) +
  geom_point()



datos_aux <- datos %>% mutate(timestamp = as.Date(timestamp))
datos_aux <- datos_aux %>% group_by(timestamp) %>% summarise(cant = sum(cnt))
datos_aux <- datos_aux %>% mutate(dia = factor(wday(timestamp, label = TRUE),
                                  levels = c("lun\\.", "mar\\.", "mié\\.",
                                             "jue\\.", "vie\\.", "sáb\\.", "dom\\."),
                                  labels = c("Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado", "Domingo")))

datos_aux <- datos_aux %>% mutate(es_finde = (dia == "Domingo" | dia == "Sabado"))

ggplot(datos_aux, aes(cant, dia)) +
  geom_point() +
...
ggplot(datos_aux, aes(cant, es_finde)) +
  geom_boxplot() +
  geom_jitter()
